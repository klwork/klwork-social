...
public class SelectUserField extends HorizontalLayout implements Field {
	...
	public SelectUserField(String caption) {
		...
		selectedUserLabel.setValue(i18nManager
				.getMessage(Messages.FORM_USER_NO_USER_SELECTED));
		selectedUserLabel
				.addStyleName(ExplorerLayout.STYLE_FORM_NO_USER_SELECTED);
		addComponent(selectedUserLabel);

		selectUserButton = new Button();
		selectUserButton.addStyleName(Reindeer.BUTTON_SMALL);
		selectUserButton.setCaption(i18nManager
				.getMessage(Messages.FORM_USER_SELECT));
		...
		selectUserButton.addListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				final SelectUsersPopupWindow window = new SelectUsersPopupWindow(
						i18nManager.getMessage(Messages.FORM_USER_SELECT),
						false);
				...
				ViewToolManager.showPopupWindow(window);
			}
		});
	...
	public void setValue(Object newValue) throws ReadOnlyException,
			ConversionException {
		wrappedField.setValue(newValue.toString());
		...
		if (newValue != null) {
			if (selectedUser == null || !selectedUser.getId().equals(newValue)) {
				...
				// selectedUser =
				// ExplorerApp.get().getUserCache().findUser((String) newValue);
			}
			selectedUserLabel.setValue(getSelectedUserLabel().toString());
			selectedUserLabel
					.addStyleName(ExplorerLayout.STYLE_FORM_USER_SELECTED);
			selectedUserLabel
					.removeStyleName(ExplorerLayout.STYLE_FORM_NO_USER_SELECTED);
		} else {
			selectedUser = null;
			selectedUserLabel.setValue(i18nManager
					.getMessage(Messages.FORM_USER_NO_USER_SELECTED));
			selectedUserLabel
					.addStyleName(ExplorerLayout.STYLE_FORM_NO_USER_SELECTED);
			selectedUserLabel
					.removeStyleName(ExplorerLayout.STYLE_FORM_USER_SELECTED);
		}
	}
	...
	public void addListener(ValueChangeListener listener) {
		wrappedField.addValueChangeListener(listener);
	}

	public void removeListener(ValueChangeListener listener) {
		wrappedField.removeValueChangeListener(listener);
	}
	...
	public void setBuffered(boolean buffered) {
		wrappedField.setBuffered(buffered);
	...
	public boolean isBuffered() {
		...
		return wrappedField.isBuffered();
	}
